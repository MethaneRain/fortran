...
Exploring the gcc PATH for the Homebrew gave: ```usr/local/Cellar/``` So all that needed to be done to have the gcc point to the Homebrew's version would be to add a PATH that points to it in the <em><h5><strong><em>end</em></strong></p5> of the ```.bash_profile``` file.
